/* This header should be included by any program using this memeory manager. ONLY things that are needed by another
   program using this MM should be defined here (POLA.) Additionally only C11 compatible syntax should be used in
   this head file (with the exception of the statment  "extern "C"" and the associated braces (Each of the three
   lines that contain the aformentioned statment and associated braces has a comment that can be used by a script to
   identify and remove them (thus rendering the file appropriate to be included in C source files.)))*/
#ifndef MM_HPP_
#define MM_HPP_
